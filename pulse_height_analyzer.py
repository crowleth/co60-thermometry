...
class HistogramTab(tk.Frame):
    ...
    def __init__(self, parent, status_var: tk.StringVar, **kw):
        ...
        self._timestamps: list[float] = []      # NEW: Store pulse times
        self._start_time: float = 0.0           # NEW: Track when the run started
    ...
    def _browse_file(self):
        ...
        # Export existing data if the run is stopped and we have data in memory
        if not self._running and self._heights:
            try:
                with open(path, "w", newline="") as f:
                    writer = csv.writer(f)
                    # MODIFIED: Write headers and include historical timestamps
        ...
        if path:
            try:
                ...
                if self._csv_file.tell() == 0:
                    # MODIFIED: Added elapsed_time_s column
    ...
    def _clear_hist(self):
        self._heights.clear()
        self._timestamps.clear() # NEW: clear timestamps
    ...
    def _poll(self):
        ...
        while not self._q.empty():
            ...
            # Find peak and calculate time
            peak = float(np.max(item)) 
            elapsed = time.time() - self._start_time  # NEW: Calculate elapsed time
            
            self._heights.append(peak)
            self._timestamps.append(elapsed)          # NEW: Store elapsed time
            self._last_waveform = item          
            
            if self._csv_writer:
                ts = datetime.datetime.now().isoformat(timespec="milliseconds")
                # MODIFIED: Write all 3 columns
